...
void addEdge(vector < vector <bool> > &graph, int i, int j) {
	graph.at(i).at(j)=graph.at(j).at(i)=1; }
...
int main() {
	...
	for (int firstVert=0; firstVert<n; firstVert++) {
		...
		for (int i=0; i<noNeighbors; i++) {
			...
			int weight;
			cin >> weight;
			addnVert(graph, pseudo, weight-2, firstVert, secondVert); } }
	int sVert;
	cout << "Enter source vertex: ";
	cin >> sVert;
	vector <int> tree = bfs(graph, sVert);
	list <int> path;
	for (int dVert=0; dVert<n; dVert++) {
		if(dVert==sVert)
			continue;
		path.clear();
		for (int i = dVert; i != -1; i=tree[i])
			path.push_front(i);
		cerr << "Shortest path from "<<sVert<< " to " <<dVert<<endl;
		for (auto i : path)	{
			if (!pseudo.at(i))
				cerr << i << " -> "; }
		cerr <<endl; } }
